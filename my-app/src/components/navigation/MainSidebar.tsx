"use client";
import { Bell, Home, LogOut, Menu, Store, User, X } from "lucide-react";
...
export default function MainSidebar({ onLogoutClick }: MainSidebarProps) {
  ...
  // 🟢 BUYER-ONLY: Navigation items - ONLY buyer routes
  const navItems = [
    { href: "/buyer/dashboard", label: "Dashboard", icon: Home },
    { href: "/buyer/marketplace", label: "Marketplace", icon: Store },
    { href: "/buyer/notification", label: "Notifications", icon: Bell },
    { href: "/buyer/profile", label: "Profile", icon: User },
  ];

  // 🟢 BUYER-ONLY: Active state logic - ONLY buyer routes
  const isActive = (href: string) => {
    if (href === "/buyer/dashboard") {
      return pathname?.startsWith("/buyer/dashboard");
    }
    if (href === "/buyer/marketplace") {
      return pathname?.startsWith("/buyer/marketplace");
    }
    if (href === "/buyer/notification") {
      return pathname?.startsWith("/buyer/notification");
    }
    if (href === "/buyer/profile") {
      return pathname?.startsWith("/buyer/profile");
    }
  ...
  const handleLogoutClick = (e: React.MouseEvent) => {
    ...
    console.log("🖱️ Buyer Logout button clicked - opening modal only");
